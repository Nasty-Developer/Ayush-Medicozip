...
const router = Router();
// Admin: list coupons
router.get("/", requireAuth, requireAdminEmail, async (_req, res) => {
    try {
...
router.post("/validate", requireAuth, async (req, res) => {
    try {
        ...
        if (orderAmount == null || typeof orderAmount !== "number" || orderAmount < 0) {
            res.status(400).json({ error: "orderAmount (number ≥ 0) is required" });
            return;
        }
        ...
        if (orderAmount < minOrder) {
            res.status(400).json({ error: `Minimum order amount is ₹${minOrder}` });
            return;
        }
        let discountAmount;
