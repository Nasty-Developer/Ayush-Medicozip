...
import { requireAuth, requireAdminEmail, isAdminEmail } from "../middlewares/authMiddleware";
const router = Router();
...
router.post("/failure", requireAuth, async (req, res) => {
    const { orderDbId } = req.body;
    if (!orderDbId) {
        res.status(400).json({ error: "orderDbId is required" });
        return;
...
router.post("/send-request", requireAuth, requireAdminEmail, async (req, res) => {
    const { orderDbId } = req.body;
    if (!orderDbId) {
        res.status(400).json({ error: "orderDbId is required" });
        return;
...
router.post("/refund", requireAuth, requireAdminEmail, async (req, res) => {
