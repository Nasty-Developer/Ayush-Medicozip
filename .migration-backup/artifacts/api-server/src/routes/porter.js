...
router.post("/estimate", requireAuth, requireAdminEmail, async (req, res) => {
    if (!isConfigured()) {
        res.status(503).json({
...
router.post("/book", requireAuth, requireAdminEmail, async (req, res) => {
    if (!isConfigured()) {
        res.status(503).json({
            error: "Porter not configured",
            hint: "Set PORTER_API_KEY in environment secrets to enable delivery booking.",
    ...
    try {
        ...
        const payload = {
            ...
            additional_comments: `${items.map((i) => `${i.medicineName ?? "Item"} ×${i.quantity ?? 1}`).join(", ")}`,
        };
...
router.get("/track/:porterOrderId", requireAuth, requireAdminEmail, async (req, res) => {
    if (!isConfigured()) {
...
router.post("/cancel", requireAuth, requireAdminEmail, async (req, res) => {
    if (!isConfigured()) {
        res.status(503).json({ error: "Porter not configured", configured: false });
        return;
    }
