...
import { usersTable, adminUsersTable } from "@workspace/db";
...
router.get("/by-firebase/:uid", requireAuth, async (req, res) => {
    try {
...
router.post("/admin/sync", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const data = req.body;
