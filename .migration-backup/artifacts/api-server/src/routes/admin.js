...
import { medicinesTable, categoriesTable, companiesTable, drugGroupsTable, inquiriesTable, testimonialsTable, faqsTable, } from "@workspace/db";
...
router.post("/companies", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const { name } = req.body;
        if (!name?.trim()) {
...
router.put("/companies/:id", requireAuth, requireAdminEmail, async (req, res) => {
    try {
...
router.delete("/companies/:id", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const id = Number(req.params["id"]);
        if (isNaN(id)) {
            res.status(400).json({ error: "Invalid id" });
...
// ── Drug Groups ───────────────────────────────────────────────────────────────
router.get("/drug-groups", async (req, res) => {
    try {
        const search = req.query["search"] ? String(req.query["search"]) : undefined;
        const page = Math.max(1, parseInt(String(req.query["page"] ?? "1"), 10));
        const limit = Math.min(200, parseInt(String(req.query["limit"] ?? "50"), 10));
...
// ── Medicines (admin) ─────────────────────────────────────────────────────────
router.get("/medicines", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const search = req.query["search"] ? String(req.query["search"]) : undefined;
        const page = Math.max(1, parseInt(String(req.query["page"] ?? "1"), 10));
        ...
        const statusCond = statusFilter === "all"
            ? undefined
            : eq(medicinesTable.status, statusFilter);
        const searchCond = search ? or(ilike(medicinesTable.name, `%${search}%`), ilike(medicinesTable.genericName, `%${search}%`)) : undefined;
        // Optional category filter (server-side)
        const categoryIdParam = req.query["categoryId"] ? Number(req.query["categoryId"]) : undefined;
        const categoryCond = categoryIdParam && !isNaN(categoryIdParam)
            ? eq(medicinesTable.categoryId, categoryIdParam)
            : undefined;
...
router.post("/medicines", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const body = req.body;
        if (!body.name?.trim()) {
            res.status(400).json({ error: "name is required" });
        ...
        const [created] = await db.insert(medicinesTable).values(body).returning();
        res.status(201).json(created);
    }
    catch (err) {
        if (err.code === "23505") {
            res.status(409).json({ error: "Medicine with this product code already exists" });
            return;
        }
...
router.put("/medicines/:id", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const id = Number(req.params["id"]);
        if (isNaN(id)) {
            res.status(400).json({ error: "Invalid id" });
...
router.delete("/medicines/:id", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const id = Number(req.params["id"]);
        if (isNaN(id)) {
            res.status(400).json({ error: "Invalid id" });
