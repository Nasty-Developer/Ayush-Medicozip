...
import { requireAuth, requireAdminEmail, type AuthenticatedRequest } from "../middlewares/authMiddleware.js";
...
router.get(
  ...
  requireAuth,
  requireAdminEmail,
  async (req: Request<{ porterOrderId: string }>, res: Response): Promise<void> => {
    if (!isConfigured()) {
