...
/** Records that the customer completed the manual UPI step.
 * A UTR is accepted for backwards compatibility but is never required.
...
export async function verifyUpiPayment(
  ...
  await authFetchJson("/api/payment/verify-upi", {
    method: "POST",
    body: JSON.stringify({
      orderDbId: orderDocId,
    }),
  });
}
