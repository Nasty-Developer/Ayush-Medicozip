    ...
    >
      ...
      <div className="relative p-4 flex flex-col flex-1">
        ...
        <Link
          ...
          className="text-sm font-bold text-foreground mb-1.5 leading-tight hover:text-secondary transition-colors"
...
export default function SpecialMedicines() {
  ...
  const { data: medicinesData, isLoading: medicinesLoading } = useQuery({
    queryKey: ["special-medicines"],
    queryFn: async () => {
      const r = await fetch("/api/medicines/special?limit=20");
  ...
  const loading = medicinesLoading;
  const sectionEnabled = homepageSettings?.specialMedicinesEnabled !== false;
  const title       = (homepageSettings?.specialMedicinesTitle       as string | undefined) ?? DEFAULTS.title;
  const description = (homepageSettings?.specialMedicinesDescription as string | undefined) ?? DEFAULTS.description;

  const items: Medicine[] = [...(medicinesData?.data ?? [])].sort((a, b) => {
