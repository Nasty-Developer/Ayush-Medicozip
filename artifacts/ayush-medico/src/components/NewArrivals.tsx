...
export default function NewArrivals() {
  ...
  const { data: medicinesData, isLoading: medicinesLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await fetch("/api/medicines/new-arrivals?limit=20");
  ...
  const sectionEnabled = homepageSettings?.newArrivalsEnabled !== false;
  const title       = (homepageSettings?.newArrivalsTitle       as string | undefined) ?? DEFAULTS.title;
  const description = (homepageSettings?.newArrivalsDescription as string | undefined) ?? DEFAULTS.description;
