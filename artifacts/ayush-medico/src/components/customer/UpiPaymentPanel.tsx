...
import { CheckCircle2, Clipboard, Copy, IndianRupee, Loader2, Smartphone } from "lucide-react";
...
export default function UpiPaymentPanel({
  ...
  const [utr, setUtr] = useState(upiTransactionId ?? "");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (trimmed.length < 4) {
      toast({ variant: "destructive", title: "Enter your UPI reference", description: "Use the UTR or transaction ID shown in your UPI app." });
      return;
    }
    setSubmitting(true);
    try {
      await submitUpiPayment({ orderDbId, upiTransactionId: trimmed });
      toast({ title: "Payment submitted for verification", description: "We’ll verify the transaction and update your order status." });
      onSubmitted?.();
    } catch (error) {
      toast({
        variant: "destructive",
        ...
        <div className="p-5">
          <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-800 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                We received your transaction reference{upiTransactionId ? ` ${upiTransactionId}` : ""}. Our pharmacy team will verify it before preparing the order.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-5">
          ...
          <div className="mt-5 pt-4 border-t border-primary/10">
            <form onSubmit={handleSubmit} className="space-y-2.5">
              <label htmlFor={`utr-${orderDbId}`} className="text-xs font-bold text-foreground flex items-center gap-1.5">
                <Clipboard size={13} className="text-primary" /> After paying, enter your UTR / transaction ID
              </label>
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  autoComplete="off"
                  className="flex-1 px-3 py-2.5 rounded-xl border border-border bg-background text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-primary/20"
                />
                <button type="submit" disabled={submitting} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-secondary text-white text-sm font-bold hover:bg-secondary/90 disabled:opacity-60 transition-colors">
                  {submitting ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />}
                  {submitting ? "Submitting…" : "Submit for verification"}
                </button>
              </div>
              <p className="text-[11px] text-muted-foreground">Submitting a reference does not mark the order paid. A pharmacist must verify it first.</p>
            </form>
          </div>
        </div>
      )}
