            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={String(lbl)}>
                  ...
                  <input type="number" min="0" step="0.01" value={String(val)}
...
export default function MedicinesPage() {
  ...
  // Keep a ref so load() can access the latest categories without being in its dep array
  const categoriesRef = useRef(categories);
  useEffect(() => { categoriesRef.current = categories; }, [categories]);

  const load = useCallback(async (q: string, p: number, catName: string = "All") => {
    setLoading(true);
    try {
      const token  = await auth?.currentUser?.getIdToken();
      const params = new URLSearchParams({ page: String(p), limit: String(PAGE_SIZE) });
      if (q.trim()) params.set("search", q.trim());
      // Server-side category filter — resolve name → id then send to API
      if (catName && catName !== "All") {
        const cat = categoriesRef.current.find((c) => c.name === catName);
  ...
  const handleSearch = (val: string) => {
    ...
    searchTimer.current = setTimeout(() => { setPage(1); load(val, 1, catFilter); }, 300);
  };

  const handlePage = (p: number) => { setPage(p); load(search, p, catFilter); };
          ...
          <div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
            ...
            <p className="text-sm font-medium">
              {search || catFilter !== "All"
                ? "No medicines match your search / category filter"
