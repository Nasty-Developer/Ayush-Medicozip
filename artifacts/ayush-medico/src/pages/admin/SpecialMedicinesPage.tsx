...
export default function SpecialMedicinesPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="flex items-start justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-secondary text-white text-sm font-semibold rounded-xl shadow-md shadow-secondary/25 hover:bg-secondary/90 transition-all flex-shrink-0"
          >
      ...
      {loading ? (
        <div className="flex items-center justify-center h-40"><Loader2 size={24} className="animate-spin text-secondary" /></div>
      ) : items.length === 0 ? (
              ...
              <div key={item.id} className="bg-card border border-border rounded-2xl p-4 shadow-sm flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0 bg-gradient-to-br from-secondary/10 to-primary/10 flex items-center justify-center border border-border">
                  {item.imageUrl ? (
                    ...
                    <span className="text-secondary font-bold text-sm">{item.name.charAt(0)}</span>
                  )}
                </div>
