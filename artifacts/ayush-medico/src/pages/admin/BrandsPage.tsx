 ...
 *   • Drag-and-drop ordering — saves order field to Firestore (mirrors CategoriesPage).
...
import { Plus, Pencil, Trash2, X, Loader2, Building2, Upload, AlertCircle, GripVertical } from "lucide-react";
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Website */}
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Website <span className="text-muted-foreground font-normal">(optional)</span></label>
...
export default function BrandsPage() {
  ...
  const handleDrop = async (e: React.DragEvent, dropIdx: number) => {
    ...
    // Persist new order (sequential × 10 leaves room for inserts)
    try {
      await Promise.all(
        reordered.map((brand, i) => updateDocument("brands", brand.id, { order: (i + 1) * 10 }))
      );
    } catch {
      toast({ variant: "destructive", title: "Failed to save order", description: "Please try again." });
    }
  };
              ...
              <div className="flex-1 min-w-0">
                ...
                {brand.website && (
                  <a href={brand.website} target="_blank" rel="noopener noreferrer"
                    className="text-[10px] text-primary/60 hover:text-primary truncate block" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <button onClick={() => handleToggle(brand)}
                  className={`px-2.5 py-1 rounded-full text-[10px] font-semibold transition-all ${
                    brand.enabled ? "bg-secondary/10 text-secondary" : "bg-muted text-muted-foreground"
                  }`}>
