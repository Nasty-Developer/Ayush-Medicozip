 ...
 *  - Medicines loaded in pages of 25 (paginated getDocs) with a "Load More"
 *    button — no longer streams the entire category at once.
...
import {
  ...
  Search, X, SlidersHorizontal, PackageSearch,
} from "lucide-react";
...
export default function CategoryDetailPage() {
  ...
  const {
    medicines, loading: medicinesLoading,
    loadingMore, hasMore, loadMore,
  } = useMedicinesByCategory(category?.name ?? "");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Breadcrumb className="mb-6">
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                <Link href="/categories" className="hover:text-primary transition-colors">Browse Medicines</Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator><ChevronRight size={14} /></BreadcrumbSeparator>
            <BreadcrumbItem>
        ...
        >
          <h2 className="text-xl font-bold text-foreground mb-6" style={{ fontFamily: "'Poppins', sans-serif" }}>
          ...
          {medicinesLoading ? (
            ...
            <div className="flex flex-col items-center gap-4 py-20 px-6 border-2 border-dashed border-border rounded-2xl bg-muted/20 text-center">
              <div className="p-4 rounded-2xl bg-muted/50">
                <Package size={36} className="text-muted-foreground/40" />
              </div>
              <div>
                <h3 className="font-semibold text-foreground mb-1">No medicines available in this category yet.</h3>
                <p className="text-sm text-muted-foreground max-w-sm">
                  Medicines appear here automatically once added or synced via MediVision Gold. No reload needed.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-3 py-16 px-6 text-center border border-dashed border-border rounded-2xl bg-muted/10">
              ...
              <p className="text-muted-foreground">No medicines match "{debouncedSearch}".</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {visibleMedicines.map((item, i) => <MedicineCard key={item.id} item={item} index={i} />)}
            </div>
          )}

          {/* ── Load More ─────────────────────────────────────────────── */}
          {!medicinesLoading && !debouncedSearch && hasMore && (
            <div className="mt-10 flex flex-col items-center gap-3">
              ...
              >
                {loadingMore ? <Loader2 size={15} className="animate-spin" /> : <Package size={15} />}
                {loadingMore ? "Loading…" : "Load more medicines"}
              </button>
              <p className="text-xs text-muted-foreground">
                {medicines.length} loaded — more available
              </p>
            </div>
          )}

          {/* All loaded */}
          {!medicinesLoading && !debouncedSearch && !hasMore && medicines.length > 0 && (
            <p className="mt-8 text-center text-xs text-muted-foreground">
              All {medicines.length} medicines loaded.
            </p>
          )}
        </motion.div>
