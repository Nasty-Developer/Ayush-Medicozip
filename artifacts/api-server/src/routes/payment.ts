 ...
 *
 * POST /api/payment/submit-upi             → customer confirms a UPI payment for review
 * POST /api/payment/verify-upi             → admin verifies a confirmed UPI payment
...
// Customer-only endpoint. A customer can confirm payment without entering a
// transaction ID, but cannot set a paid/verified status. Admin verification
// remains the only paid transition.
router.post(
  "/submit-upi",
  requireAuth,
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    if (!orderDbId || (transactionId && (transactionId.length < 4 || transactionId.length > 120))) {
      res.status(400).json({ error: "orderDbId is required; transaction ID is optional" });
      return;
    }
    ...
    try {
      ...
      const payment = {
        ...
        upiId: UPI_ID,
        ...(transactionId ? { upiTransactionId: transactionId } : {}),
        submittedAt: new Date().toISOString(),
        customerConfirmedAt: new Date().toISOString(),
      };
      const [updated] = await db
        .update(ordersTable)
        .set({ payment, status: "payment-verification-pending", updatedAt: new Date() })
        .where(eq(ordersTable.id, Number(orderDbId)))
        .returning();
...
// Admin-only transition. The customer confirmation is intentionally not treated
// as proof of payment; only this endpoint can move the payment to paid.
router.post(
  "/verify-upi",
  requireAuth,
  requireAdminEmail,
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (payment.status !== "verification-pending") {
        res.status(409).json({ error: "Customer payment confirmation is required before verification" });
        return;
      }
      if (["cancelled", "delivered", "returned", "refunded"].includes(order.status)) {
        res.status(409).json({ error: "This order can no longer be verified" });
        return;
      }
      ...
      const updatedPayment = {
        ...
        verifiedBy: req.firebaseUser?.email ?? req.firebaseUser?.uid,
      };
      const [updated] = await db
        .update(ordersTable)
        .set({ payment: updatedPayment, status: nextOrderStatus, updatedAt: new Date() })
        .where(eq(ordersTable.id, Number(orderDbId)))
        .returning();

      logger.info({ orderDbId, orderId: order.orderId }, "Admin verified UPI payment");
      res.json({ success: true, order: updated });
    } catch (err) {
      logger.error({ err }, "POST /payment/verify-upi failed");
      res.status(500).json({ error: "Failed to verify UPI payment" });
    }
  }
);
