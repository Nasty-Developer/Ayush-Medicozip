...
import { companiesTable, categoriesTable, drugGroupsTable, medicinesTable, stockTable, settingsTable, syncSessionsTable, uploadChunksTable, } from "@workspace/db";
...
function hasValidJobLease(job, now = Date.now()) {
    return Boolean(job?.status === "running" &&
...
async function saveJobToDb(job) {
    try {
        ...
        const updated = await db
            ...
            .where(and(eq(settingsTable.key, SYNC_JOB_KEY), sql `value->>'ownerId' = ${job.ownerId}`))
...
async function claimPersistedJobLease(job) {
    ...
    return db.transaction(async (tx) => {
        ...
        const staleJob = existingJob?.status === "running" ? existingJob : null;
        await tx
...
const ACTIVE_SESSION_STATUSES = ["uploading", "running"];
function isActiveSessionStatus(status) {
    return status === "uploading" || status === "running";
}
function makeJob(sessionId, firebaseUid) {
    const now = Date.now();
    return {
        id: `sync_${randomUUID()}`,
        sessionId,
...
async function createSyncSession(firebaseUid) {
    return db.transaction(async (tx) => {
      ...
      FOR UPDATE
    `);
        const existingRow = activeRows.rows[0];
        const existingStatus = typeof existingRow?.status === "string" ? existingRow.status : "";
        const existingExpiresAt = existingRow?.expires_at instanceof Date
            ? existingRow.expires_at
            : new Date(String(existingRow?.expires_at ?? ""));
        if (typeof existingRow?.id === "string" &&
...
async function loadOwnedSession(sessionId, firebaseUid, statuses) {
    await db
    ...
    const rows = await db
        ...
        .where(and(eq(syncSessionsTable.id, sessionId), eq(syncSessionsTable.firebaseUid, firebaseUid), inArray(syncSessionsTable.status, [...statuses]), gt(syncSessionsTable.expiresAt, sql `CURRENT_TIMESTAMP`)))
        .limit(1);
...
async function storeChunkForSession(input) {
    return db.transaction(async (tx) => {
        const activeSession = await tx
            ...
            .where(and(eq(syncSessionsTable.id, input.sessionId), eq(syncSessionsTable.firebaseUid, input.firebaseUid), eq(syncSessionsTable.status, "uploading"), gt(syncSessionsTable.expiresAt, sql `CURRENT_TIMESTAMP`)))
...
async function claimSyncSession(sessionId, firebaseUid) {
    const rows = await db
        .update(syncSessionsTable)
        .set({
        status: "running",
        ...
        .where(and(eq(syncSessionsTable.id, sessionId), eq(syncSessionsTable.firebaseUid, firebaseUid), eq(syncSessionsTable.status, "uploading"), gt(syncSessionsTable.expiresAt, sql `CURRENT_TIMESTAMP`)))
        .returning({ id: syncSessionsTable.id });
...
async function refreshRunningSyncSessionLease(sessionId, firebaseUid) {
    const rows = await db
        ...
        .where(and(eq(syncSessionsTable.id, sessionId), eq(syncSessionsTable.firebaseUid, firebaseUid), eq(syncSessionsTable.status, "running"), gt(syncSessionsTable.expiresAt, sql `CURRENT_TIMESTAMP`)))
        .returning({ id: syncSessionsTable.id });
    return rows.length > 0;
}
...
// ── Chunk assembly ─────────────────────────────────────────────────────────────
const VALID_FILE_KEYS = ["product_sdf", "stock_sdf", "company_sdf", "category_sdf", "drug_sdf"];
/**
...
async function assembleFile(sessionId, fileKey) {
    const rows = await db
        ...
        .where(and(eq(uploadChunksTable.sessionId, sessionId), eq(uploadChunksTable.fileKey, fileKey)))
...
// ── Main importer ─────────────────────────────────────────────────────────────
async function runImport(job, buffers, session) {
    const t0 = Date.now();
    let terminalSessionStatus = "failed";
    let lastSessionHeartbeatAt = 0;
    const refreshRunningLeaseIfDue = async (force = false) => {
        const now = Date.now();
        if (!force &&
    ...
    try {
        ...
        if (uniqueCategories.length) {
            await db
                ...
                .values(uniqueCategories.map((name) => ({
                name,
                slug: slugify(name),
                icon: "💊",
                color: "primary",
        ...
        for (let i = 0; i < medBatches.length; i++) {
            ...
            const values = batch.map((m) => ({
                ...
                stockStatus: m.stockQty > 10 ? "in_stock"
                    : m.stockQty > 0 ? "low_stock"
                        : "out_of_stock",
                stockQty: m.stockQty,
                status: (m.available && m.name !== "DELETED") ? "active" : "deleted",
            }));
...
// ── Routes ────────────────────────────────────────────────────────────────────
/** GET /api/sync/status */
router.get("/status", requireAuth, requireAdminEmail, async (_req, res) => {
 ...
 */
router.post("/session", requireAuth, requireAdminEmail, async (req, res) => {
 ...
 */
router.post("/chunk", requireAuth, requireAdminEmail, (req, res, next) => { CHUNK_UPLOAD(req, res, next); }, async (req, res) => {
 ...
 */
router.post("/start", requireAuth, requireAdminEmail, async (req, res) => {
...
router.delete("/cancel", requireAuth, requireAdminEmail, async (req, res) => {
    const firebaseUid = req.firebaseUser?.uid;
    if (!firebaseUid) {
        res.status(401).json({ error: "Authenticated Firebase user is required." });
        return;
