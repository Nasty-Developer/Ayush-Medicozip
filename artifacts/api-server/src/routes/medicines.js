...
import { medicinesTable, categoriesTable, companiesTable, } from "@workspace/db";
...
async function fetchMedicinesWithJoins(where, limit = 50, offset = 0, extraOrder) {
    const rows = await db
        .select({
        m: medicinesTable,
...
router.get("/medicines/featured", async (req, res) => {
    try {
...
router.get("/medicines/new-arrivals", async (req, res) => {
    try {
        const limit = parseInt(String(req.query.limit ?? "12"), 10);
...
router.get("/medicines/special", async (req, res) => {
    try {
        const limit = parseInt(String(req.query.limit ?? "12"), 10);
...
router.get("/medicines/:id/related", async (req, res) => {
    try {
        const id = parseInt(req.params.id ?? "", 10);
        if (isNaN(id)) {
            res.status(400).json({ error: "Invalid medicine id" });
...
router.get("/categories", async (_req, res) => {
    try {
        const rows = await db
            ...
            .from(categoriesTable)
            .leftJoin(medicinesTable, and(eq(medicinesTable.categoryId, categoriesTable.id), eq(medicinesTable.status, "active")))
