...
import { productsTable, inventoryTable } from "@workspace/db";
...
router.get("/:id/inventory", requireAuth, requireAdminEmail, async (req, res) => {
    try {
...
router.put("/:id/inventory", requireAuth, requireAdminEmail, async (req, res) => {
    try {
        const productId = Number(req.params["id"]);
        if (isNaN(productId)) {
            res.status(400).json({ error: "Invalid id" });
