...
router.get("/medicines/:id", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  try {
...
router.get("/medicines/:id/related", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id ?? "", 10);
    if (isNaN(id)) { res.status(400).json({ error: "Invalid medicine id" }); return; }
...
router.get("/category/:slug", async (req: Request<{ slug: string }>, res: Response): Promise<void> => {
  try {
