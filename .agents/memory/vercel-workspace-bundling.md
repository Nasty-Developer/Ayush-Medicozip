...
**How to apply:** Keep Replit/Render's artifact build separate from the root `api/build.mjs` Vercel build. Avoid explicit Express `IRouter` annotations in routes when Vercel's type pass resolves the interface without router methods; let `Router()` infer the type. Validate that the generated Vercel bundle contains no `@workspace/` imports, can be imported as an Express app, and that generated bundle output stays ignored.